...
class CurrencyConvertViewController: UIViewController {
    ...
    var selectedCurrency: SelectedCurrency = .init(code: "INR", rate: 100)
    ...
    @IBAction func conversionTaped(_ sender: Any) {
        fetchCurrencyDetails()
    }
    ...
    func fetchCurrencyDetails() {
        ...
        CurrencyViewModelApi().fetchCurrencyDetails { [unowned self] result in
            ...
            self.updateValues(with: amountEnterTF.text ?? "1")
        }
    }
}
